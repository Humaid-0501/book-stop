...
public class ApiResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Category CategoryName(@PathParam("category-name") String categoryName,
                                          @Context HttpServletRequest httpRequest) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Book> suggestedBooksByCategoryNameAndLimit(@PathParam("category-name") String categoryName,
                                                   @QueryParam("limit") @DefaultValue("3") int limit,
                                                   @Context HttpServletRequest request) {
        try {
            Category category =  categoryDao.findByName(categoryName);
